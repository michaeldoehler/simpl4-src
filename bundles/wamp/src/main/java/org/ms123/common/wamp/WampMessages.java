...
public class WampMessages {
	...
	public static abstract class WampMessage {
		...
		public abstract JsonNode toObjectArray(ObjectMapper mapper) throws WampError;

		public static WampMessage fromObjectArray(ArrayNode messageNode) throws WampError {
	...
	public static class HelloMessage extends WampMessage {
		...
		static class Factory implements WampMessageFactory {
			@Override
			public WampMessage fromObjectArray(ArrayNode messageNode) throws WampError {
				if (messageNode.size() != 3 || !messageNode.get(1).isTextual() || !messageNode.get(2).isObject())
					throw new WampError(ApplicationError.INVALID_MESSAGE);
	...
	public static class WelcomeMessage extends WampMessage {
		...
		static class Factory implements WampMessageFactory {
			@Override
			public WampMessage fromObjectArray(ArrayNode messageNode) throws WampError {
				if (messageNode.size() != 3 || !messageNode.get(1).canConvertToLong() || !messageNode.get(2).isObject())
					throw new WampError(ApplicationError.INVALID_MESSAGE);
	...
	public static class AbortMessage extends WampMessage {
		...
		static class Factory implements WampMessageFactory {
			@Override
			public WampMessage fromObjectArray(ArrayNode messageNode) throws WampError {
				if (messageNode.size() != 3 || !messageNode.get(1).isObject() || !messageNode.get(2).isTextual())
					throw new WampError(ApplicationError.INVALID_MESSAGE);
	...
	public static class CallMessage extends WampMessage {
		...
		static class Factory implements WampMessageFactory {
			...
			public WampMessage fromObjectArray(ArrayNode messageNode) throws WampError {
				...
				return new CallMessage(requestId, options, procedure, arguments, argumentsKw);
			}
		}
	}
