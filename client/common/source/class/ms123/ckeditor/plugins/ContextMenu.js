...
qx.Class.define("ms123.ckeditor.plugins.ContextMenu", {
	/**
	 * @ignore(CKEDITOR)
	 */
	defer: function () {
						...
						addTarget: function (element, nativeContextMenuOnCtrl) {
							...
							if (CKEDITOR.env.webkit) {
								var holdCtrlKey, onKeyDown = function (event) {
									holdCtrlKey =
									CKEDITOR.env.mac ? event.data.$.metaKey : event.data.$.ctrlKey;
								},
						...
						open: function (offsetParent, corner, offsetX, offsetY, domEvent) {
							...
							function (listenerFn) {
								...
								if (listenerItems) {
									...
									Object.keys(listenerItems).forEach(

									function (itemName) {
										// Determine the label and command for this menu item
										var item = this.editor.getMenuItem(itemName);
										if (item && item.command && this.editor.getCommand(item.command)) {
											...
											menuItem.addListener("execute", function (e) {
												...
												(cmd ? cmd() : this.editor.execCommand(commandName));
											}, this);
										}
									}, this);
