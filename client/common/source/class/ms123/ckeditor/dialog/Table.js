...
/**
	...
	@ignore(CKEDITOR.NODE_ELEMENT)
	@ignore(CKEDITOR.POSITION_AFTER_START)
	@ignore(CKEDITOR.env);
	@ignore(CKEDITOR.env.ie);
	@ignore(CKEDITOR.tools.trim);
	@ignore(CKEDITOR.dom.text)
	@ignore(CKEDITOR.dom.range)
	@ignore(CKEDITOR.dom.walker.nodeType);
 */

qx.Class.define("ms123.ckeditor.dialog.Table", {
	extend: ms123.ckeditor.dialog.AbstractDialog,

	construct: function (ckrte_editor,param) {
		// Call the superclass constructor
		this.base(arguments, this.tr("Table Properties"), ckrte_editor,param);
	},

	members: { /** The selected table, if we're editing an existing table */
		 ...
		 * @ignore(CKEDITOR)
		 */
		_create: function (ckrte_editor, param) {
			var selection;
			var ranges;
			console.log("_create." + param);
			var isNew = this._isNew = param == "table";
			...
			if ( !isNew ) {
				...
				if ( selected && selected.is( 'table' ) ){
					...
					table = this.getCkEditor().elementPath( ranges[ 0 ].getCommonAncestor( true ) ).contains( 'table', 1 );
				}
				this._selectedElement = table;
			}
			this._table = this._selectedElement;
			if (this._table==null) {
			...
			// First column of fields
			{
				...
				setup: function (elem) {
					if (!isNew) {
						...
						return this._table.getAttribute("data-cke-rows") || "";
					}
			...
			},
			{
				...
				setup: function (elem) {
					if (!isNew) {
						elem.setEnabled(false);
						return this._table.getAttribute("data-cke-cols") || "";
					}
			...
			},
			{
				...
				setup: function (elem) {
					...
					for (var row = 0; row < this._table.$.rows.length; row++) {
					...
					if ((this._table.$.tHead !== null)) {
						return (this.getUserData("hasColumnHeaders") ? "both" : "row");
					}
					else {
						return (this.getUserData("hasColumnHeaders") ? "col" : "none");
					}
				}
			},
			{
			...
			{
				label: this.tr("No Output Border"),
				model: "boolNoOutBorder",
				type: qx.ui.form.CheckBox,
				value: false,
				col: 0,
				setup: function (elem) {
					var s = this._table.getAttribute("summary");
					return ( s != null && s.indexOf("nob=true")!=-1) ? true : false;
				}
			},
			{
			...
			// Second column of fields
			{
				...
				setup: function (elem) {
					if (!isNew) {
						return this._table.getAttribute("width") || "";
					}
			...
			},
			{
				...
				setup: function (elem) {
					if (!isNew) {
						return this._table.getAttribute("height") || "";
					}
			...
			},
			{
				...
				setup: function (elem) {
					if (!isNew) {
						return (this._table.getAttribute("cellSpacing") || "");
					}
			...
			},
			{
				...
				setup: function (elem) {
					if (!isNew) {
						return (this._table.getAttribute("cellPadding") || "");
					}
			...
			// Define the properties for the dual-width fields at the bottom
			{
				...
				setup: function (elem) {
					...
					if (this._table) {
						var nodeList = this._table.getElementsByTag("caption");
						if (nodeList.count() > 0) {
		...
		_onOk: function (model) {
			...
			if (!this.getUserData("hasColumnHeaders") && (headers == "col" || headers == "both")) {
			...
			function (attr) {
				if (attr.valid && !qx.lang.Array.contains(attr.valid, info[attr.fieldName])) {
				...
				} else if (attr.fieldName == 'boolNoOutBorder') {
					table.setAttribute("summary", "nob="+info[attr.fieldName]);
				} else {
					table.removeAttribute(attr.attrName);
				}
			});
			...
			if (this._caption.getEnabled()) {
				...
				if (caption && caption.length > 0) {
					...
					captionElement.append(new CKEDITOR.dom.text(caption, ckeditor.document));
				}
