...
qx.Class.define("ms123.ckeditor.dialog.Image", {
	extend: ms123.ckeditor.dialog.AbstractDialog,

	construct: function (ckrte_editor) {
		this.__storeDesc = ckrte_editor.__storeDesc;
	...
	members: {
		...
		_create: function (ckrte_editor) {
			...
			var namespace = this.__storeDesc.getNamespace();
			this.setDialogConfiguration([{
				label: this.tr("Image file"),
				model: "txtDataUrl",
				type:	ms123.form.ResourceSelectorField,
				col: 0,
				set: {
					type:[ "image/png", "image/jpg", "image/svg+xml"],
					editable:false,
					convertIn:function(val){
						console.log("convertIn:"+val);
						if( val && val.indexOf(":") != -1){
							return val.split(":")[1];	
						}
						/*if( val && val.indexOf("%3A") != -1){
							return val.split("%3A")[1];	
						}*/
						return val;
					},
					convertOut:function(val){
						console.log("convertOut:"+val);
						if( val){
							return namespace+"/repo:"+val;
						}	
						return val;
					},
			...
			},
			{
				...
				setup: function (elem) {
					if (this._selectedImage) {
						return (
						this._selectedImage.getAttribute("width") || this._getPx(this._selectedImage.getStyle("width")) || "");
					}
			...
			},
			{
				...
				setup: function (elem) {
					if (this._selectedImage) {
						return (
						this._selectedImage.getAttribute("height") || this._getPx(this._selectedImage.getStyle("height")) || "");
					}
			...
			// Add the image previewer
			{
				label: null,
				model: "imgPreview",
				type: ms123.ckeditor.dialog.ImagePreviewer,
				col: 3,
				row: 0,
				rowSpan: 5,
				setup: function (elem) {
			...
			this._elements["txtDataUrl"].bind("value", this._previewer, "data");
			...
			this._elements["cmbAlign"].bind("modelSelection[0]", this._previewer, "align");
		},
		...
		_configureImageAttributes: function (image, info) {
			...
			info.txtDataUrl && info.txtDataUrl.length > 0 ? image.setAttribute("src", info.txtDataUrl) : image.removeAttribute("src");
			info.txtDataUrl && info.txtDataUrl.length > 0 ? image.setAttribute("data-cke-saved-src", info.txtDataUrl) : image.removeAttribute("data-cke-saved-src");

			// Add the alt property
			info.txtAlt && info.txtAlt.length > 0 ? image.setAttribute("alt", info.txtAlt) : image.removeAttribute("alt");
			...
			if (info.txtBorder && info.txtBorder.length > 0) {
				image.setStyle("border-width", CKEDITOR.tools.cssLength(info.txtBorder));
			...
			if (info.txtHSpace && info.txtHSpace.length > 0) {
				image.setStyle("margin-left", CKEDITOR.tools.cssLength(info.txtHSpace));
				image.setStyle("margin-right", CKEDITOR.tools.cssLength(info.txtHSpace));
			}
			else {
			...
			if (info.txtVSpace && info.txtVSpace.length > 0) {
				image.setStyle("margin-top", CKEDITOR.tools.cssLength(info.txtVSpace));
				image.setStyle("margin-bottom", CKEDITOR.tools.cssLength(info.txtVSpace));
			}
			else {
