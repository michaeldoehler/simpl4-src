...
/**
	...
	@ignore(CKEDITOR.plugins.link)
	@ignore(CKEDITOR.STYLE_INLINE)
	@ignore(CKEDITOR.dom.text)
 */

qx.Class.define("ms123.ckeditor.dialog.Link", {
	...
	members: { /** Saved selection, to avoid changes while a modal window appears */
		...
		_onOk: function (model) {
			...
			if (!element || !element.hasAttribute("href")) {
				...
				if (ranges.length == 1 && ranges[0].collapsed) {
					text = new CKEDITOR.dom.text((selectedText && selectedText.length > 0 ? selectedText : info.txtUrl), ckeditor.document);
