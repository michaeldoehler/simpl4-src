...
qx.Class.define("ms123.ckeditor.dialog.AbstractDialog", {
	extend: qx.ui.window.Window,

	construct: function (title, ckrte_editor,param) {
		// Call the superclass constructor
		this.base(arguments, title);

		// Create the window
		this._create(ckrte_editor,param);
	},

	properties: { /** The CkEditor editor object */
		ckEditor: {
			init: null
		},
	...
	members: { /** Model skeleton */
		...
		_create: function (columns) {
			...
			if (qx.core.Environment.get("qx.debug")) {
				this.assert((typeof columns == "undefined" || columns === 1 || columns === 2), "The only supported values for the 'columns' parameter " + "are 1 or 2. The parameter may also be excluded, to " + "yield the default value of 2.");
			}
			...
			if (columns !== 1) {
				container.add(new qx.ui.core.Spacer(30), {
					row: 0,
					column: 2
				});
			}
			...
			function (item) {
				if (item.type === qx.ui.form.SelectBox) {
					this._controller.addTarget(item.input, "modelSelection[0]", item.model, true);
				} else if (item.type === qx.ui.form.ComboBox) {
					this._controller.addTarget(item.input, "value", item.model, true);
				} else if (item.type === qx.ui.form.TextField) {
					this._controller.addTarget(item.input, "value", item.model, true);
				} else if( item.type === ms123.form.ResourceSelectorField){
					this._controller.addTarget(item.input, "value", item.model, true);
				} else if (item.type === qx.ui.form.CheckBox) {
					this._controller.addTarget(item.input, "value", item.model, true);
				}
			}, this);
			...
			hBox.add(new qx.ui.core.Spacer(null, 1), {
				flex: 1
			});
			...
			this._okListener = this._okButton.addListenerOnce("execute", function (e) {
			...
			this._cancelListener = this._cancelButton.addListenerOnce("execute", function (e) {
				this.close();
		...
		_addField: function (container, row, item, data) {
			...
			if (item.type === qx.ui.form.SelectBox) {
				...
				function (listItem) {
					item.input.add(new qx.ui.form.ListItem(listItem.label, null, listItem.model));
				});
			}

			if (item.type === qx.ui.form.ComboBox) {
				item.items.forEach(

				function (listItem) {
					item.input.add(new qx.ui.form.ListItem(listItem.value, null, listItem.value));
				});
			}
		...
		_makeElement: function (type, bDocument) {
			return new CKEDITOR.dom.element(type, (bDocument ? this.getCkEditor().document : undefined));
		}
	}
});
