 ...
 */
qx.Class.define("ms123.widgets.smart.model.TreeTable", {
	...
	members: {
		...
		initTree: function (view) {
			var nodeArr = {
				0: qx.ui.treevirtual.MTreePrimitive._getEmptyTree()
			};
		 ...
		 */
		addBranch: function (view, nodeArr, rowId, parentNodeId, label, bOpened, bHideOpenCloseButton, bHeader, icon) {
			// Create a new node.
			var nodeId =
			qx.ui.treevirtual.MTreePrimitive._addNode(
			nodeArr, parentNodeId, label, bOpened, bHideOpenCloseButton, qx.ui.treevirtual.MTreePrimitive.Type.BRANCH, icon, null, rowId);
		 ...
		 */
		addLeaf: function (view, nodeArr, rowId, parentNodeId, label, icon) {
			var nodeId =
			qx.ui.treevirtual.MTreePrimitive._addNode(
			nodeArr, parentNodeId, label, false, false, qx.ui.treevirtual.MTreePrimitive.Type.LEAF, icon, null, rowId);
		 ...
		 */
		__inorder: function (view, srcNodeArr, srcRowArr, destNodeArr, destRowArr, nodeId, level) {
			var child = null;
			var childNodeId;
			...
			for (var i = 0; i < numChildren; i++) {
				...
				if (child.bOpened) {
					...
					this.__inorder(view, srcNodeArr, srcRowArr, destNodeArr, destRowArr, childNodeId, level + 1);
				}
			}
		}
	}
