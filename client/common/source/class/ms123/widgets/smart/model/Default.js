 ...
 */
qx.Class.define("ms123.widgets.smart.model.Default", {
	extend: qx.ui.table.model.Simple,

	//include: smart.MSmartUnitTests,
	// unit testing code
	/**
	 * 
	...
	members: {
		...
		_applyView: function (view, old, fireEvent, force, preserve_selection) {
			...
			if (view == old && !force) return;

			if (view >= this.__views.length) {
				throw new Error("_applyView: view out of bounds: " + view + " (0.." + (this.__views.length - 1) + ")");
			}
			...
			if (preserve_selection) this.__saveSelection(old);
			...
			if (fireEvent) this.__notifyDataChanged(view);
		 ...
		 */
		newView: function (fFilter, context, advanced) {
			...
			var viewData = {
				...
				fFilter: (fFilter ? qx.lang.Function.bind(fFilter, context) : null),
		...
		__saveSelection: function (view) {
			...
			function (row) {
				...
				// selected rows array.
				this.__selectedRows.push(this.getValue(this.__selectionIndex, row, view));
			}, this);
		},
		...
		__restoreSelection: function (view) {
			...
			if (this.__selectedRows == null || !this.__selectionModel || this.__selectionIndex < 0 || this.__selectionIndex >= this.getColumnCount()) {
				return;
			}
		...
		getRowArray: function (view, alternate) {
			...
			if (typeof alternate == "boolean") {
				return (alternate ? this.__alternate_backingstore[view] : this.__backingstore[view]);
			}
			else {
				var ret =
		...
		getRowData: function (rowIndex, view, copy) {
			...
			return (copy ? rowArray[rowIndex].slice(0) : rowArray[rowIndex]);
		},
		...
		__insertRows: function (view, rows, runFilters, alreadySorted, updateAssociationMaps) {
			...
			if (viewData.advanced.fPreInsertRows) {
				viewData.advanced.fPreInsertRows.call(viewData.context, view, this.getRowArray(view), rows, this);
			}
			...
			if (viewData.advanced.fPostInsertRows) {
				viewData.advanced.fPostInsertRows.call(viewData.context, view, this.getRowArray(view), rows, this);
			}
		},
		...
		__set: function (columnIndex, rowIndex, V, view) {
			...
			if (columnIndex >= 0 && columnIndex < columns) {
				...
				reinsert = (this.isSorted() && (columnIndex === this.getSortColumnIndex()));
			}
			else {
		...
		addRows: function (rowArr, copy, fireEvent) {
			...
			//@@@MS
			if( comparator.columnIndex != -1) A.sort(comparator);
		...
		removeReferencedRows: function (rows, fireEvent) {
			...
			for (var v = 0; v < this.__views.length; v++) {
				...
				if (viewData.advanced.fPreRemoveRows) {
					viewData.advanced.fPreRemoveRows.call(viewData.context, v, this.getRowArray(v, false), rows, this);
				}
				...
				if (viewData.advanced.fPostRemoveRows) {
					viewData.advanced.fPostRemoveRows.call(viewData.context, v, this.getRowArray(v), this);
				}
			}
		...
		getComparator: function (columnIndex, ascending) {
			...
			if (sortMethods) {
				...
				comparator = (ascending ? Simple._defaultSortComparatorAscending : Simple._defaultSortComparatorDescending);
			}
			else {
				comparator = (ascending ? Simple._defaultSortComparatorInsensitiveAscending : Simple._defaultSortComparatorInsensitiveDescending);
			}
		...
		_resort: function () {
			if (this.isSorted()) {
				this.sortByColumn(this.getSortColumnIndex(), this.isSortAscending(), /*force:*/ true);
			}
		},
