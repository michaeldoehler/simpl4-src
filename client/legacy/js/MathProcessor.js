...
with({
	...
	p.parse = function (e) {
		...
		for (;
		(n = o.pop()) != undefined; n[0] = _.o[n[0]](isNaN_c(n[2][0]) ? _.f(n[2][0]) : n[2][0], isNaN_c(n[3][0]) ? _.f(n[3][0]) : n[3][0]));
		return x[0];
	};
	...
	p.RPN = function (e) {
		...
		while (e.length) {
			...
			e[0] in _.s && (c = _.s[e[0]] > _.s[c[0]] ? ((c[3] = (x = c[3], c[2]))[1][2] = [e.shift(), c, x, 0])[2][1] = c[2] : r == c ? (r = [e.shift(), , c, 0])[2][1] = r : ((r[2] = (x = r[2], [e.shift(), r, , 0]))[2] = x)[1] = r[2]);
		}
		return r;
	};
	p.v = function (e) {
		...
		if ("0123456789.".indexOf(e[0]) + 1) {
			...
			if (((j = e[0].toLowerCase()) >= "a" && j <= "z") || j == "_") {
				for (i = 0;
				((j = e[++i].toLowerCase()) >= "a" && j <= "z") || j == "_" || (j >= 0 && j <= 9););
				if (j == "(") {
