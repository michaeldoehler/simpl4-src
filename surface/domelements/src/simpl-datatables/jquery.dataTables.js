...
(/** @lends <global> */function( $ ) {
	...
	function _fnFeatureHtmlLength ( settings )
	{
      ...
      //select[0][ i ] = new Option( language[i], lengths[i] ); //@@@MS Firefox problem
	...
	function _fnCalculateColumnWidths ( oSettings )
	{
		...
		else
		{
			...
			for ( i=0 ; i<visibleColumns.length ; i++ ) {
				column = columns[ visibleColumns[i] ];
				if( i < headerCells.length){ //@@@MS, sometimes more visibleColumns as headerCells,??? 
	...
	function _fnThrottle( fn, freq ) {
		...
		return function () {
			...
			if ( last && now < last + frequency ) {
				...
				last = now;
				//@@@MS catch exception,reason, when the table invisible
				try{
					fn.apply( that, args );
				}catch(e){
