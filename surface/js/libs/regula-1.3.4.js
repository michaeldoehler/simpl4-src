...
(function(root, factory) {
	...
	function initializePublicValidators(constraintDefinitions) {
		for (var constraintName in constraintDefinitions)
			if (constraintDefinitions.hasOwnProperty(constraintName)) {
	...
	function compoundValidator(params, currentGroup, compoundConstraint, callback) {
		...
		function mergeParams(composingConstraintParams, compoundConstraintParams) {
			...
			for (var paramName in composingConstraintParams)
				if (composingConstraintParams.hasOwnProperty(paramName) && paramName != "__size__") {
			...
			if (compoundConstraintParams.length > 0) {
				for (var paramName in params)
					if (params.hasOwnProperty(paramName) && paramName != "__size__") {
	...
	function bind(options) {
		...
		if (typeof options === "undefined" || !options) {
			...
			if (config.enableHTML5Validation && DOMUtils.supportsHTML5Validation()) {
				result = BindingService.bindHTML5ValidationConstraints({
					element: null
				});
			}
			...
			if (result.successful) {
				result = BindingService.bindAfterParsing({
					element: null
				});
			}
